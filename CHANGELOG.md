...
- **[Core]** Debug surface for answering "why didn't my shortcut fire": `ShortcutRegistry.debugEvents` publishes a per-keystroke `ShortcutDebugEvent` (dispatched with stack depth / chord advanced / key-repeat suppressed / no match), and `activationSnapshot` exposes the live activation stack in router order with duplicates preserved — unlike `activeBindings()`, which collapses both. Gated by `isDebugRecording` (default `false`); while off the event path costs one optional test per keystroke. Nothing is stored. See <doc:Debugging>.
